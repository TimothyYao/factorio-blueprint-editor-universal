...
URL imports (`?source=https://…`) need a CORS proxy that **does not run** on GitHub Pages; paste a blueprint string or use `?source=<bpstring>` instead. The pastebin samples above are too large to embed in a URL (Pages returns 414 past ~8 KB), so the direct-string links use compact vanilla prints that still load. See [`docs/github-pages.md`](./docs/github-pages.md).
