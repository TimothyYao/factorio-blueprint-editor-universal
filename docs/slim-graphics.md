...
{ "id": "vanilla-2.0-slim", "label": "Vanilla 2.0 (slim)",
  "variantOf": "vanilla-2.0", "graphics": "slim",
  "artifacts": ["editor"] }          // no browser/ tier — FIB is unaffected
```
...
```jsonc
{ "__base__/graphics/entity/assembling-machine-2/….basis":
    { "crop": [x, y, w, h],   // region of the ORIGINAL image this file contains
      "scale": 0.5 } }        // factor applied after cropping
```

`getTexture` applies the transform when present (`x' = (x-crop.x)*scale`, …)
and PixiJS sprite scale compensates by `1/scale`; a missing entry or a
missing `textures.json` means identity — full packs are untouched and old
deploys keep working. Requests that fall outside the cropped region (a code
path the census missed, or a future editor feature) fail soft to the
existing missing-texture placeholder and log loudly.
...
The graphics *source* becomes a setting, orthogonal to the pack choice:
...
- [ ] Editor: `textures.json` transform support in `getTexture` (+ soft-fail
...
- [ ] Rect report: extract the census enumeration into a reusable script
  emitting per-file union bboxes for a pack.
- [ ] Exporter: slim mode (crop + 0.5× downscale + `textures.json`),
