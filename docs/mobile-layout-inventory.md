...
| **Entity-info** (`EntityInfoPanel`)             | top-right of the (inset) canvas        | 270 × 270 (grows w/ recipe) | fit + clamp; re-anchors on canvas inset                                             | Starts below the reserved top band on mobile (#89 Phase 1) — no longer under the pill       |
...
| **Active-project pill** (`#active-project`, #50) | **top-center** (`top: 8px, left: 50%`)          | `min(50vw, 360px)` × ~28px                   | 5       | **Always visible**; its footprint is now **reserved** on mobile — `viewportRegions.ts` measures it (+ the corner logo) and insets the canvas below via `setViewportInsets({ top })` (#89 Phase 1), so it no longer covers Pixi panels |
...
`editor.setViewportInsets({ top })`, so on mobile the canvas — and every
panel anchored to its top — starts _below_ the chrome. One writer per edge:
the rail owns `left`, this module owns `top`. Desktop stays full-bleed (the
chrome and the top-right panels don't meet at normal window widths).
Guarded by the disjointness ratchet in `e2e/panels.spec.ts` ("top band").
