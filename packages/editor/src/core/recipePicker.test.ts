...
describe('namesMissingFromInventoryLayout', () => {
    it.skipIf(!havePackData('space-exploration'))(
        'reports only names absent from the layout',
        () => {
...
describe('SE iron-ingot-to-plate is pickable (regression)', () => {
    it.skipIf(!havePackData('space-exploration'))(
        'assembling-machine-2 accepts it and the layout now places it',
        () => {
            loadData(readPackData('space-exploration'))
            ...
            expect(namesMissingFromInventoryLayout(accepted)).not.toContain(
...
describe('layout has no duplicate names (regression)', () => {
    // it.each has no per-case skipIf; a plain loop registers the same tests.
    for (const pack of ['vanilla-2.0', 'space-age', 'space-exploration'])
        it.skipIf(!havePackData(pack))(pack, () => {
...
describe.each(['vanilla-2.0', 'space-age', 'space-exploration'])(
    ...
    pack => {
        it.skipIf(!havePackData(pack))(
            'never offers a hidden recipe (recipe-unknown, *-recycling, …)',
            () => {
                loadData(readPackData(pack))
                ...
                expect(
                    hiddenOffered,
                    `hidden recipes offered: ${hiddenOffered.join(', ')}`
                ).toEqual([])
...
describe.each(['vanilla-2.0', 'space-age', 'space-exploration'])(
    ...
    pack => {
        it.skipIf(!havePackData(pack))(
            'layout ∪ orphans is exhaustive for crafting machines',
            () => {
                loadData(readPackData(pack))
                const present = layoutNames()
                const gaps: string[] = []
