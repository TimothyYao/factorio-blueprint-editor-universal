...
export class EntitySprite extends Sprite {
    ...
    public static getParts(
        ...
        // entity colours carry a ≈ 0.5, render it semi-transparent too. Lamps
        // skip it as well: their colour only shows when lit, and the editor
        // renders the unlit sprite. Keep the blanket only for entities with no
        // handling of their own (rolling stock), where a whole-sprite wash is
        // the best approximation we have.
        const entityColor =
            entity.type === 'train-stop' || entity.type === 'lamp' ? undefined : rawEntityColor
